...
| Topic                                    | Description                              |
| :--------------------------------------- | :--------------------------------------- |
| <a href="https://github.com/exitmsconfig/hack.python#attackwifipy-一个获取wifi密码的工具">attackWiFi.py | 一个获取wifi密码的工具 |
| <a href="https://github.com/exitmsconfig/hack.python#图解https">图解https</a> | 图解https |
| <a href="https://github.com/exitmsconfig/hack.python#图解hsts">图解HSTS</a> | 图解HSTS |
| <a href="https://github.com/exitmsconfig/hack.python#tcpportforwardpy-端口转发tool">TcpPortForward.py</a> | 端口转发tool |
| <a href="https://github.com/exitmsconfig/hack.python#typeecho-get-shell--typeecho反序列化漏洞利用">typeEchoShell.py|typeEchoShell.php</a> | typeEcho反序列化漏洞利用 | 
| <a href="https://github.com/exitmsconfig/hack.python#获取所有连接过的wifi密码win平台">wifi密码</a> | 获取所有连接过的wifi密码(Win平台) | 
| <a href="https://github.com/exitmsconfig/hack.python#zipattackpy--zakpy-zip加密文件暴力破解">zipattack.py|zak.py</a> | zip加密文件暴力破解 |
| <a href="https://github.com/exitmsconfig/hack.python#createDictpy-生成一个简单的密码破解字典">createDict.py</a> | 生成一个简单的密码破解字典 |
| <a href="https://github.com/exitmsconfig/hack.python#portscanpy-多线程端口扫描器">PortScan.py</a> | 多线程端口扫描器 |
| <a href="https://github.com/exitmsconfig/hack.python#sshattackpy-多线程ssh密码暴力破解">sshAttack.py</a> | 多线程ssh密码暴力破解 |
| <a href="https://github.com/exitmsconfig/hack.python#ftpattackpy-多线程ftp密码暴力破解">ftpAttack.py</a> | 多线程ftp密码暴力破解 |
| <a href="https://github.com/exitmsconfig/hack.python#synfloodpy-一个简单的-tcp-syn-洪水攻击-python版">synFlood.py</a> | synFlood TCP SYN 洪水攻击 |
| <a href="https://github.com/exitmsconfig/hack.python#ntpdenialservicepy-一个使ntp拒绝服务的poc代码">ntpdenialservice.py</a> | 一个使ntp拒绝服务的poc代码 |
| <a href="https://github.com/exitmsconfig/hack.python#分享一个sql注入的技巧">SQL Inject</a> | 分享一个SQL注入的技巧 |
| <a href="https://github.com/exitmsconfig/hack.python#attacksmbwin10py-一个smb漏洞的poc代码">attackSmb/win10.py</a> | 一个SMB漏洞的Poc |
| <a href="https://github.com/exitmsconfig/hack.python#badtunnel-pocrb-badtunnel-跨网段劫持">badtunnel-poc.rb</a> | badtunnel 跨网段劫持 |
| <a href="https://github.com/exitmsconfig/hack.python#sqlinjetctpy-一个延时注入的验证">sqlInjetct.py</a> | 一个延时注入的验证 |
| <a href="https://github.com/exitmsconfig/hack.python#关于ssrf漏洞-与-dns域传送漏洞">SSRF & DNS</a> | SSRF漏洞 & DNS域传送漏洞 |
| <a href="https://github.com/exitmsconfig/hack.python#使用python构造一个fastcgi协议请求内容发送给php-fpm--nginxiis7解析漏洞原理">fpm.py</a> | Nginx（IIS7）解析漏洞原理 |
| <a href="https://github.com/exitmsconfig/hack.python#morsepy-摩斯密码加解密">morse.py</a> | 摩斯密码加解密 |
| <a href="https://github.com/exitmsconfig/hack.python#crawlpy-轻量级图片爬虫">crawl.py</a> | 轻量级图片爬虫 |
| <a href="https://github.com/exitmsconfig/hack.python#wooyun_indexpy-1000个php代码审计案例20167以前乌云公开漏洞---增加索引">wooyun_index.py</a> | 1000个PHP代码审计案例(2016.7以前乌云公开漏洞)---增加索引 |
| <a href="https://github.com/exitmsconfig/hack.python#proxy_crawlget_proxypy--ocr_imgpy-反爬虫代理服务器抓取实现方式">proxy_crawl/get_proxy.py & proxy_crawl/ocr_img.py </a> |反爬虫代理服务器抓取实现 |
...
查看帮助
➜  hack.python git:(master) ✗ py attackWiFi.py -h
Usage: use:
...
Mac环境：
	...
	其他操作系统获取方式可自行Google
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/ssid.png)
```
上一步获得了ssid和bssid信息
执行获取密码：
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/https.png)
```
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/hsts.png)
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/win_wifi_cmd.png)
...
执行：
➜  hack.python git:(master) ✗ python ntpDenialService.py 127.0.0.1 1111
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/attackSmb/attacksmb.png)
...
帮助：
➜  hack.python git:(master) ✗ py sqlInjetct.py
-----Usage:-----
...
1，cd 1000php
2，执行下载我的脚本：wget https://raw.githubusercontent.com/exitmsconfig/hack.python/master/wooyun_index.py -O wooyun_index.py
3，执行：mkdir css && mkdir js 
4，执行：wget https://github.com/exitmsconfig/hack.python/raw/master/res/style.css -O style.css -P css/
5，执行：wget https://github.com/exitmsconfig/hack.python/raw/master/res/jquery-1.4.2.min.js -O jquery-1.4.2.min.js -P js/
6，执行：python wooyun_index.py 会生成漏洞索引列表，此时会生成index.html文件，打开这个文件即可
效果如下：
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/1000php.png)
![](https://github.com/exitmsconfig/hack.python/blob/master/img/1000php_detail.png)
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/page_detail.png)
![](https://github.com/exitmsconfig/hack.python/blob/master/img/crawl_png.png)
![](https://github.com/exitmsconfig/hack.python/blob/master/img/img_list.png)
![](https://github.com/exitmsconfig/hack.python/blob/master/img/proxy_list.png)
...
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/code_v1.png)

总结文档：+ [验证码识别v1+HTTP协议Range特性分析.pdf](https://github.com/exitmsconfig/hack.python/blob/master/res/doc/验证码识别v1+HTTP协议Range特性分析.pdf)
```
HTTP协议Range特性分析(多线程文件下载器实现):
```
+ [多线程文件下载器实现](https://github.com/exitmsconfig/Py#nice_downloadpy-多线程文件下载器)


### 基于机器学习(TensorFlow)的复杂验证码识别
![](https://github.com/exitmsconfig/hack.python/blob/master/img/code_v2.png)
```
运用机器学习算法时，如果不理解实现原理，先套接口先实现功能，识别算法是通用的。一般处理不同验证码，有不同的处理策略。
分类算法举例：（具体：https://github.com/exitmsconfig/Py）
    01，knn   (k点邻近算法）
    02，svm（支持向量机，十分复杂）
    ...
...
验证码预测截图:
```
![](https://github.com/exitmsconfig/hack.python/blob/master/img/cnn_test.png)

总结文档：+ [基于机器学习(TensorFlow)的复杂验证码识别.pdf](https://github.com/exitmsconfig/hack.python/blob/master/res/doc/基于机器学习(TensorFlow)的复杂验证码识别.pdf)
